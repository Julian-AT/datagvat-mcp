...
export function getToolPaths(platform: Platform): Record<ToolName, string> {
  ...
  if (platform === 'darwin') {
    return {
      ...
      'cline': path.join(homeDir, 'Library/Application Support/Code/User/globalStorage/saoudrizwan.claude-dev/settings/cline_mcp_settings.json')
    };
  }
  ...
  return {
    ...
    'cline': path.join(homeDir, '.config/Code/User/globalStorage/saoudrizwan.claude-dev/settings/cline_mcp_settings.json')
  };
}
