...
export async function initCommand(
	...
	try {
		...
		const detectionSpinner = ui
			.spinner("  Checking for Claude Desktop, Continue, and Cline...")
			.start();
		...
		if (detectedTools.length === 0) {
			...
			console.log("");
			console.log(
				ui.dim("Install one of these tools and run the installer again."),
			);
			console.log("");
			return;
		}
		...
		if (options.tool) {
			...
			if (!specificTool) {
				...
				ui.info(
					`Detected tools: ${detectedTools.map((t) => t.name).join(", ")}`,
				);
				process.exit(1);
			}
		...
		// Interactive checkbox selection
			...
			const selectionValidation =
				ToolSelectionSchema.safeParse(selectedToolNames);
		...
		console.log("");
		const configSpinner = ui
			.spinner("  Updating configuration files...")
			.start();
