...
export async function updateCommand(
	...
	try {
		...
		console.log("");
		const detectionSpinner = ui
			.spinner("  Checking for existing configurations...")
			.start();
		...
		for (const tool of configuredTools) {
			ui.listItem(
				`${ui.cyan(tool.name)} ${ui.dim(`(${tool.configPath})`)}`,
				true,
			);
		}
		console.log("");
		...
		if (options.tool) {
			...
			if (!specificTool) {
				...
				ui.info(
					`Configured tools: ${configuredTools.map((t) => t.name).join(", ")}`,
				);
				process.exit(1);
			}
		...
		if (updateResult.skipped > 0) {
			ui.info(
				`Skipped ${updateResult.skipped} tool(s) (no changes or cancelled)`,
			);
		}
		if (updateResult.failed > 0) {
			ui.error(
				`Failed to update ${updateResult.failed} tool(s): ${updateResult.failedTools.join(", ")}`,
			);
			process.exit(1);
		}
		console.log("");
...
async function updateTool(
	...
	try {
		...
		if (err instanceof Error && "code" in err && err.code === "ENOENT") {
			...
			throw new Error(
				`Invalid JSON in config file: ${configPath}. Fix syntax or run: datagvat-mcp init --tool ${tool.name}`,
			);
		} else if (err instanceof Error && "code" in err && err.code === "EACCES") {
			throw new Error(
				`Permission denied reading config: ${configPath}. Check file permissions.`,
			);
		}
		throw err;
	}
	...
	const approved = await previewConfigUpdate(oldConfig, newConfig, {
		yes: options.yes,
	});
	...
	try {
		...
		if (err instanceof Error && "code" in err && err.code === "EACCES") {
			throw new Error(
				`Permission denied writing config: ${configPath}. Check file permissions or run with administrator/sudo privileges.`,
			);
		}
		throw err;
	}
