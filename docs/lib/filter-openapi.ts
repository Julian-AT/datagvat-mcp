...
function normalizeContent(
  ...
  const filtered = Object.fromEntries(Object.entries(content).filter(([k]) => allowed.has(k)));
...
function normalizeOperationContent(op: Operation, allowed: Set<string>): Operation {
  let req = op.requestBody;
  if (req && typeof req === 'object' && !Array.isArray(req)) {
    const reqObj = req as Record<string, unknown>;
    if (reqObj.content && typeof reqObj.content === 'object' && !Array.isArray(reqObj.content)) {
      req = {
        ...reqObj,
        content: normalizeContent(reqObj.content as Record<string, unknown>, allowed),
      };
    }
  }
  ...
  if (res && typeof res === 'object' && !Array.isArray(res)) {
    ...
    for (const [code, r] of Object.entries(res)) {
      if (r && typeof r === 'object' && !('$ref' in r)) {
        ...
        if (rObj.content && typeof rObj.content === 'object' && !Array.isArray(rObj.content)) {
          newRes[code] = {
            ...rObj,
            content: normalizeContent(rObj.content as Record<string, unknown>, allowed),
          };
        } else {
          newRes[code] = r;
        }
...
function normalizeComponents(document: Document, allowed: Set<string>): void {
    ...
    for (const v of Object.values(reqBodies)) {
      if (v && typeof v === 'object' && !('$ref' in v)) {
        ...
        if (obj.content && typeof obj.content === 'object' && !Array.isArray(obj.content)) {
          obj.content = normalizeContent(obj.content as Record<string, unknown>, allowed);
        }
      }
    }
  }
...
function isInternalOperation(
  ...
  if (internalTags.length > 0 && op.tags?.some((t: string) => internalTags.includes(t))) {
    return true;
  }

  if (op.operationId && (options.excludeOperationIds?.includes(op.operationId) ?? false)) {
    return true;
  }

  if (options.excludePathPrefixes?.some((prefix) => path.startsWith(prefix))) {
    return true;
  }

  if (options.excludeDescriptionContaining && op.description) {
    if (op.description.toLowerCase().includes(options.excludeDescriptionContaining.toLowerCase())) {
      return true;
    }
  }
...
export function filterOpenAPISchema(
  ...
  const allowedMedia = new Set(opts.allowedMediaTypes ?? [...DEFAULT_SUPPORTED_MEDIA_TYPES]);
  ...
  for (const [path, pathItem] of Object.entries(paths)) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      if (isInternalOperation(op, item, path, opts)) {
        ...
        (filteredItem as Record<string, unknown>)[method] = normalizeOperationContent(
          op,
          allowedMedia
        );
      }
