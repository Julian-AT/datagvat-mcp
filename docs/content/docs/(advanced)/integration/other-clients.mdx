...
data.gv.at MCP Server works with any MCP 1.0-compatible client. This guide covers integration with popular alternatives to Claude Desktop.

## Supported Clients

data.gv.at MCP Server is compatible with:
...
3. Add data.gv.at MCP Server server configuration
...
The `@austria-data` prefix tells Continue to use your data.gv.at MCP Server server.
...
data.gv.at MCP Server uses FastMCP for both server and client. Using the FastMCP client library simplifies custom integrations by abstracting protocol details and providing type-safe APIs.
...
# Import data.gv.at MCP Server server instance
from app.server import mcp

async def use_austria_mcp():
    """Direct in-memory connection to data.gv.at MCP Server server."""
...
To integrate data.gv.at MCP Server with your own MCP client:
...
# 1. Start data.gv.at MCP Server server
