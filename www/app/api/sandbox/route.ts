...
export async function POST(request: Request) {
  ...
  const { sandboxId, messageId, chatId, title, code, outputs, hasApprovedOnce, e2bSandboxId } =
    body;
  ...
  try {
    ...
    if (
      ...
      console.warn(
        '[API /sandbox POST] Message not found yet (race condition), returning 422 to trigger retry'
      );
