...
export function useSandboxes() {
  const { data: sandboxes, mutate: setSandboxes } = useSWR<UISandbox[]>('sandboxes', null, {
    fallbackData: [],
  });
  ...
  const closeSandbox = useCallback(
    (sandboxId: string) => {
      setSandboxes((current) => (current || []).filter((s) => s.sandboxId !== sandboxId));
    },
    [setSandboxes]
  );
  ...
  const updateSandbox = useCallback(
    (sandboxId: string, updates: Partial<UISandbox>) => {
      setSandboxes((current) =>
        (current || []).map((s) => (s.sandboxId === sandboxId ? { ...s, ...updates } : s))
      );
    },
    [setSandboxes]
  );

  const activeSandbox = useMemo(() => sandboxes?.[sandboxes.length - 1], [sandboxes]);
