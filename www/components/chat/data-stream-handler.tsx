...
export function DataStreamHandler() {
  ...
  useEffect(() => {
    ...
    console.log(
      '[DataStreamHandler] Processing',
      newDeltas.length,
      'deltas:',
      newDeltas.map((d) => d.type)
    );
    setDataStream([]);
    ...
    for (const delta of newDeltas) {
      ...
      if (pendingSandboxRef.current.isBuilding) {
        ...
        if (delta.type === 'data-finish' && pendingSandboxRef.current.sandboxId) {
          console.log(
            '[DataStreamHandler] data-finish received, creating sandbox:',
            pendingSandboxRef.current.sandboxId
          );
